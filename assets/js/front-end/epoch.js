/** globals angular, jQuery, EPOCH_VARS, Visibility **/
var Epoch = Epoch || {};

Epoch.app = angular.module( 'epoch', ['ngResource', 'ngSanitize', 'ngStorage' ] ).config( function( $rootScope ) {
    $rootScope.postID = angular.element.attr( 'data-epoch-post-id' );
    $rootScope.total = angular.element.attr( 'data-epoch-post-id' );
    $rootScope.totalPages = 0;
    $rootScope.open = angular.element.attr( 'data-epoch-open' );
    $rootScope.highest = 0;
});

Epoch.app.controller( 'comments', ['$scope', '$rootScope', '$http', '$sce', 'commentStore', function( $scope, $rootScope, $http, $sce, commentStore ){
    var page;
    $scope.post_comments = {};
    $scope.partials = EPOCH_VARS.partials;
    if ( _.isEmpty( $scope.comment ) ) {
        $scope.comment = {
            author: EPOCH_VARS.user,
            content: '',
            parent: 0,
            post: $scope.postID
        };
    }


    /**
     * Check if comments are open
     *
     * @since 2.0.0
     */
    $http({
        url: EPOCH_VARS.api.posts + $rootScope.postID,
        cache: true
    } ).then( function( res )  {
        if( 'open' == res.data.comment_status ) {
             $scope.comments_open = true;
        }else{
            $scope.comments_open = false;
        }
    });

    $scope.translations = EPOCH_VARS.translations;



    /**
     * Get comments and update model
     *
     * @since 2.0.0
     */
    var getComments = function(){
        $scope.comments = function( page) {
            commentStore(page);
        }
    };

    //run at start
    getComments();


    /**
     * Should we hide previous button?
     *
     * @since 2.0.0
     *
     * @returns {boolean}
     */
    $scope.showPrevPage = function() {
        if( 1 != page  ){
            return true;
        }
    };

    /**
     * Should we hide next button?
     *
     * @since 2.0.0
     *
     * @returns {boolean}
     */
    $scope.showNextPage = function() {
        if( page < totalPages ) {
            return true;
        }

    };

    /**
     * Callback for getting previous page of comments
     *
     * @since 2.0.0
     *
     */
    $scope.prev = function(){
        page--;
        getComments();

    };

    /**
     * Callback for getting next page of comments
     *
     * @since 2.0.0
     */
    $scope.next = function(){
        page++;
        getComments();
    };

    /**
     * Have URL for comment author?
     *
     * @since 2.0.0
     *
     * @param id
     * @returns {boolean}
     */
    $scope.hasAuthorURL = function( id ){
        var match = find( id );
        if ( !_.isEmpty( match ) ) {
            if ( !_.isEmpty( match.author_url ) ){
                return true;
            }
        }

    };

    /**
     * Check if comment is approved
     *
     * @since 2.0.0
     *
     * @param id
     * @returns {boolean}
     */
    $scope.isApproved = function( id ){
        var match = find( id );
        if ( !_.isEmpty( match ) ) {
            if ( 'approved' == match.status ){
                return true;
            }
        }
    };

    /**
     * Approve a comment
     *
     * @since 2.0.0
     */
    $scope.approve = function(){
        var comment = this.comment;
        return update_status( comment, 'approved' );
    };

    /**
     * Unpprove a comment
     *
     * @since 2.0.0
     */
    $scope.unApprove = function(){
        var comment = this.comment;
        return update_status( comment, 'hold' );
    };

    /**
     * Spam a comment
     *
     * @since 2.0.0
     */
    $scope.spam = function(){
        var comment = this.comment;
        return update_status( comment, 'spam' );
    };

    /**
     * Delete a comment
     *
     * @since 2.0.0
     */
    $scope.delete = function(){
        var comment = this.comment;
        var id = comment.id;
        $http({
            url: EPOCH_VARS.api.comments + id + '?_wpnonce=' + EPOCH_VARS.nonce,
            method: 'DELETE'
        }).then( function( res ) {
            $rootScope.totalPages =  res.headers('x-wp-totalpages');
            $rootScope.total = res.headers( 'w-wp-total' );
        }, function errorCallback( res ) {
            var error = res.data.message;
        });
    };

    /**
     * Find a comment in the current comments object by comment ID
     *
     * @since 2.0.0
     * @param id
     */
    var find = function( id ){
        var found = _.filter( $scope.post_comments, _.matches( {
            id: id
        } ) );
        if( !_.isEmpty( found ) ) {
            return found[0];
        }
    };

    var update_status = function( comment, status ){
        var id = comment.id;
        $http({
            url: EPOCH_VARS.api.comments + id + '?_wpnonce=' + EPOCH_VARS.nonce,
            params: {
                id: id,
                status: status
            },
            method: 'POST'
        }).then( function( res ) {
            $rootScope.totalPages  =  res.headers('x-wp-totalpages');
            $rootScope.total = res.headers( 'w-wp-total' );
            console.log( res );
        }, function errorCallback( res ) {
            var error = res.data.message;
        });

    };

    /**
     * Click handler for moving form form for inline comments
     *
     * @since 2.0.0
     */
    $scope.moveForm = function() {
        var id = this.comment.id;
        moveForm( id );
    };

    /**
     * Move handler for moving form form for inline comments
     *
     * @since 2.0.0
     */
    var moveForm = function ( id ) {
        angular.element( '#epoch-reply' ).detach().appendTo( '#comment-' + id );
        $scope.comment.parent = id;
    };

    /**
     * Edit comment
     *
     * @since 2.0.0
     *
     * @todo this
     */
    $scope.editComment = function(){
        $scope.comment = this.comment;
        $scope.comment.content = this.comment.content.rendered;
        moveForm( this.comment.id );

    };

    /**
     * Show edit link?
     *
     * @since 2.2.0
     *
     * @returns {boolean}
     */
    $scope.showEdit = function(){
        if( this.comment.author == EPOCH_VARS.user ){
            return true;
        }
    };

    /**
     * Put comment form back when canceling
     *
     * @since 2.0.0
     */
    $scope.cancel = function() {
        $scope.comment = {};
        angular.element( '#epoch-reply' ).detach().appendTo( '#epoch-comment-before' );
    };


}]);

Epoch.app.factory( 'commentStore', [ '$scope', '$rootScope', '$localStorage', '$http', function($scope, $rootScope, $localStorage, $http ) {

    var commentIDs;


    $scope.total = $localStorage.total || 0;

    $scope.$watch('total', function() {
        $localStorage.total = $scope.total;
    });

    $scope.$watch('total', function( newValue, oldValue ) {
        if ( newValue > oldValue ) {
           getComments();
        }
    });
    var initialized = false;
    var init = function(){
        if( $rootScope.total > localCount ){
            getComments(); //foreach here?
        }else{
            //??
        }

        initialized = true;

    };

    var localCount = function(){
        return $localStorage.comments.length || 0;
    };

    $scope.$watch(function() {
        return angular.toJson( $localStorage );
    }, function() {
        $scope.total = $localStorage.total;
    });



    if ( 1==9 && EPOCH_VARS.live_mode ) {
        Visibility.every( EPOCH_VARS.epoch_options.interval, function () {
            $http({
                url: EPOCH_VARS.api.comment_count + $rootScope.postID
            } ).then( function( res ){
                if( res.count != $scope.total ){
                    $scope.total = res.count;
                }
            });
        });
    }



    /**
     * Get comments and update model
     *
     * @since 2.0.0
     */
    var getComments = function( page ){
        $http({
            url: EPOCH_VARS.api.comments,
            params: {
                page: page,
                post: $rootScope.postID,
                epoch: true,
                epochHighest: $rootScope.highest
            },
            cache: false
        }).then( function( res ) {
            $rootScope.totalPages =  res.headers('x-wp-totalpages');
            $rootScope.total = res.headers( 'x-wp-total' );
            $rootScope.highest = res.headers( 'x-epoch-highest' );
            if ( 0 < res.data.length ) {
                var id;

                _.each( res.data, function( comment, i ) {
                    id = comment.id;
                    $localStorage.comments[ id ] = comment;
                    $localStorage.comments[ id ].avatar = $localStorage.comments[ id ].author_avatar_urls[ 96 ];
                    if ( -1 == commentIDs.indexOf( id ) ) {
                        commentIDs.push( {
                            id: id,
                            page: page
                        } );
                    }
                    if ( !_.isEmpty( comment.children ) ) {
                        _.forEach( comment.children, function ( child, cI ) {
                            $localStorage.comments[ id ].children[ child.id ] = $localStorage.comments[ id ].children[ child.id ];

                        } );
                    }
                });

            }

        });
    };

    return function( page ){
        if( ! initialized ) {
            init();
        }

        var found = _.filter( $scope.post_comments, _.matches( {
            page: page
        } ) );

        console.log( found );
        return found;
    }
}]);

/**
 * Controller for comment form
 *
 * @since 2.0.0
 */
Epoch.app.controller( 'commentForm', [ '$scope', '$rootScope', '$http', function( $scope, $rootScope, $http ){
    $scope.translations = EPOCH_VARS.translations;
    $scope.logout_link = EPOCH_VARS.logout_link;

    /**
     * Handle comment submission
     *
     * @since 2.0.0
     */
    $scope.submit = function(){
        $http({
            url: EPOCH_VARS.api.comments + '?_wpnonce=' + EPOCH_VARS.nonce,
            params: $scope.comment,
            method: 'POST'
        }).then( function( res ) {
            $scope.comment = {};
            totalPages =  res.headers('x-wp-totalpages');
            total = res.headers( 'w-wp-total' );
            console.log( res );
        }, function errorCallback( res ) {
            var error = res.data.message;
        });
    }

}]);





